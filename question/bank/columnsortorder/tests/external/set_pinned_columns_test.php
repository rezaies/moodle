 ...
 * @covers \qbank_columnsortorder\external\set_pinned_columns
 */
class set_pinned_columns_test extends external_function_testcase {

    protected $testclass = '\qbank_columnsortorder\external\set_pinned_columns';

    protected $setting = 'pinnedcols';

    /**
     * Generate a random list of pinned column names.
     *
     * @return array
     */
    protected function generate_test_data(): array {
        ...
        $pinnedcolumns = array_slice($questionlistcolumns, rand(0, count($questionlistcolumns) - 1));
        return array_map(fn($column) => $column->name, $pinnedcolumns);
    }
}

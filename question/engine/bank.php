...
abstract class question_bank {
    ...
    public static function load_question_data($questionid, $allowbroken = false) {
        ...
        if (isset($questiondata->loadingexception)) {
            $debuginfo = "Message: {$questiondata->loadingexception->message}";
            $debuginfo .= "\n ";
            $debuginfo .= "Stack trace: {$questiondata->loadingexception->tracestring}";
            if ($allowbroken) {
                debugging($debuginfo, DEBUG_NORMAL);
            } else {
                throw new moodle_exception('errorloadingquestion', 'question', '', $questionid, $debuginfo);
            }
        }
        return $questiondata;
    }

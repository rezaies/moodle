...
var DIALOGUE_NAME = 'Moodle dialogue',
    ...
    CAN_RECEIVE_FOCUS_SELECTOR = 'input:not([type="hidden"]):not([disabled]):not([tabindex="0"]):not([tabindex="-1"]),' +
        'a[href]:not([disabled]):not([tabindex="0"]):not([tabindex="-1"]),' +
        'button:not([disabled]):not([tabindex="0"]):not([tabindex="-1"]),' +
        'textarea:not([disabled]):not([tabindex="0"]):not([tabindex="-1"]),' +
        'select:not([disabled]):not([tabindex="0"]):not([tabindex="-1"]),' +
        '[tabindex]:not([disabled]):not([tabindex="0"]):not([tabindex="-1"])',
    FORM_SELECTOR = 'form';
        ...
        if (e.attrName === 'visible') {
            ...
            if (!e.prevVal && e.newVal) {
                ...
                require(['core/local/aria/focuslock'], function(FocusLockManager) {
                    ...
                    var firstFocusableBodyField = this.bodyNode.one(CAN_RECEIVE_FOCUS_SELECTOR);
                    if (firstFocusableBodyField) {
                        firstFocusableBodyField.focus();
                    }
                }.bind(this));

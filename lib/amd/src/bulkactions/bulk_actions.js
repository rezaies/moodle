...
export default class BulkActions {
    ...
    async renderBulkActions() {
        const maxButtons = 5; // Total buttons to be shown, including 'More' button if needed.
        const data = {
        ...
        const showMoreButton = bulkActions.length > maxButtons;

        // Get all bulk actions and render them in order.
        const actions = await Promise.all(
            bulkActions.map((bulkAction, index) => bulkAction.renderBulkActionTrigger(showMoreButton && (index >= maxButtons - 1)))
        );

        // Separate rendered actions into data.actions and data.moreactions in the correct order.
        actions.forEach((actionTrigger, index) => {
            if (showMoreButton && (index >= maxButtons - 1)) {

...
export default class BulkAction {
     ...
     * @param {boolean} secondGradeAction Whether the action is a second grade action. Second grade actions are put in a dropdown.
     * @param {number} index The index of the action.
     * @returns {Promise}
     */
    renderBulkActionTrigger(secondGradeAction = false, index) {
        throw new Error(`renderBulkActionTrigger(${secondGradeAction}, ${index}) must be implemented in ${this.constructor.name}`);
    }
}

 ...
 * @param stdClass|int $userorid The $user object or just its id. This defaults to the global $USER object.
 * @return array An array of courses, groups, and user to load calendar events for based upon filters
 */
function calendar_set_filters(array $courseeventsfrom, $ignorefilters = false, $userorid = null) {
    global $CFG, $DB, $USER;
 ...
 */
function calendar_get_default_courses($courseid = null, $fields = '*', $canmanage = false, $userid = null) {
    global $CFG, $USER;
...
function calendar_get_legacy_events($tstart, $tend, $users, $groups, $courses,
    ...
    // Otherwise current logged in user is used - See MDL-58769.
    if (is_array($userparam) && count($userparam) == 1) {
        \core_calendar\local\event\container::set_beneficiary_user($userparam[0]);
    }

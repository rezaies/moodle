...
abstract class activity_dates {
    ...
    public static function get_dates_for_module(cm_info $cm, int $userid): array {
        $cmdatesclassname = static::get_cm_dates_classname($cm->modname);
        if (!$cmdatesclassname) {
            return [];
        }
     ...
     * @return string|null
     */
    private static function get_cm_dates_classname(string $modname): ?string {
        $cmdatesclass = "mod_{$modname}\\cm_dates";

...
class dates extends activity_dates {
    ...
    protected function get_dates(): array {
        ...
        $lessonrecord = $DB->get_record('lesson', ['id' => $this->cm->instance]);
        $lessonobj = new lesson($lessonrecord);

        // Apply overrides.
        $lessonobj->update_effective_access($this->userid);

        $timeopen = $lessonobj->available ?? null;
        $timeclose = $lessonobj->deadline ?? null;
        $now = time();
        $dates = [];

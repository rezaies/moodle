...
class assign_grading_table extends table_sql implements renderable {
    ...
    public function col_grade(stdClass $row): string {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            ...
            $gradecontainer = $this->output->container($gradebutton . $separator . $displaygrade);

            $menu = new action_menu();
            $menu->set_owner_selector('.gradingtable-actionmenu');
            $menu->set_boundary('window');
            $menu->set_kebab_trigger(get_string('gradeactions', 'assign'));
            $menu->set_additional_classes('pl-2 ml-auto');
            ...
            $contextualmenucontainer = $this->output->container($this->output->render($menu), 'd-flex w-100');
    ...
    public function col_status(stdClass $row) {
        ...
        $actionmenucontainer = $this->output->container($actionmenu, 'd-flex w-100');
        ...
        $submissoninfocontainer = $this->output->container($submissioninfo, 'submissioninfo');

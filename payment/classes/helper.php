...
class helper {
    ...
    public static function get_available_gateways(string $component, string $paymentarea, int $itemid): array {
        ...
        $cost = static::get_cost($component, $paymentarea, $itemid);
        $account = new account($cost->get_account_id());
    ...
    public static function gateways_modal_link_params(string $component, string $paymentarea, int $itemid,
        ...
        return [
            ...
            'data-itemid' => $itemid,
            'data-cost' => static::get_cost_as_string($cost->get_amount(), $cost->get_currency()),
     ...
     */
    private static function get_component_payment_provider_class_name(string $component) {
        $providerclass = "$component\\payment\\provider";

        if (class_exists($providerclass)) {
            $rc = new \ReflectionClass($providerclass);
            if ($rc->implementsInterface(local\callback\provider::class)) {
                return $providerclass;
            }
        }

        throw new \coding_exception("$component does not have an eligible implementation of payment callback provider.");
    }
     ...
     * @param string $paymentarea
     * @param int $itemid An internal identifier that is used by the component
     * @return local\entities\cost
     */
    public static function get_cost(string $component, string $paymentarea, int $itemid): local\entities\cost {
        $providerclass = static::get_component_payment_provider_class_name($component);

        return component_class_callback($providerclass, 'get_cost', [$paymentarea, $itemid]);
    }
    ...
    public static function get_gateway_configuration(string $component, string $paymentarea, int $itemid,
        ...
        $cost = self::get_cost($component, $paymentarea, $itemid);
        $gateway = null;
        $account = new account($cost->get_account_id());
